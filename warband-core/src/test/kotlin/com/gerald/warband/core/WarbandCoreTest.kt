...
class WarbandCoreTest {
    ...
    private fun state(reserve: Double = 18.0, pool: Double = 18.0): CoreSnapshot {
    ...
    @Test fun `public squad plan is the exact dispatch composition primitive`() {
        fun planned(): Pair<SquadPlan, CoreSnapshot> {
            val state = state()
            WarbandCore.transition(state, CoreFrame(0L, commands = listOf(CoreCommand.Manufacture("warband", 2))), catalog, rules)
            val warband = state.warbands.getValue("warband")
            val plan = WarbandCore.planSquad(state, warband, state.officers.getValue("captain"), catalog, 12.0, rules)
    ...
    @Test fun `assignment and selected officer are Core owned`() {
        ...
        WarbandCore.transition(state, CoreFrame(0L, listOf(near, far)), catalog, rules)
    ...
    @Test fun `readiness accounts for a preferred expensive lead and distinct support`() {
        ...
        assertEquals(0.0, WarbandCore.raidBudget(state, warband, officer, expensiveCatalog, rules))
        warband.raidPool = 13.0
        assertEquals(13.0, WarbandCore.raidBudget(state, warband, officer, expensiveCatalog, rules))
    }
    ...
    @Test fun `automatic dispatch travels materializes fights and returns conservatively`() {
        ...
        WarbandCore.transition(
            state,
            CoreFrame(0L, listOf(player), combat = listOf(CombatObservation(campaign.id, 8.0, 1.0, 12.0, 0.8, 0.8, setOf(victim)))),
            catalog,
            rules,
        )
    ...
    @Test fun `idle active campaign requests snapshot return`() {
        ...
        val result = WarbandCore.transition(state, CoreFrame(rules.idleReturnTicks), catalog, rules)
    ...
    @Test fun `physical outcomes and captured snapshots reconcile through Core only`() {
        ...
        val outcome = WarbandCore.transition(
            state,
            CoreFrame(0L, outcomes = listOf(CampaignOutcomeObservation(campaign.id, CampaignOutcomeKind.SURVIVING_DEFEAT, "repelled"))),
            catalog,
            rules,
        )
        ...
        WarbandCore.transition(
            state,
            CoreFrame(0L, snapshots = listOf(CampaignSnapshotResult(
                campaign.id,
        ...
        WarbandCore.transition(state, CoreFrame(3L * rules.travelTicksPerChunk), catalog, rules)
        assertEquals(CampaignPhase.RESOLVED, campaign.phase)
    ...
    @Test fun `reconciliation honors configured aggression bounds`() {
        ...
        WarbandCore.transition(state, CoreFrame(boundedRules.travelTicksPerChunk), catalog, boundedRules)
        assertEquals(CampaignPhase.RESOLVED, campaign.phase)
    ...
    @Test fun `resolved campaign history does not duplicate returned equipment ownership`() {
        ...
        WarbandCore.validate(state, catalog, rules)
    }
    ...
    @Test fun `combat updates warband captain and empirical threat`() {
        ...
        val result = WarbandCore.transition(
            state,
            CoreFrame(0L, combat = listOf(CombatObservation(campaign.id, 2.0, 6.0, 14.0, 0.2, 0.2))), catalog, rules,
        )
        assertTrue(result.events.any { it.type == "combat_observed" })
    ...
    @Test fun `material ledger is exact and invalid state fails closed`() {
        ...
        val result = WarbandCore.transition(state, CoreFrame(0L, commands = listOf(CoreCommand.Manufacture("warband", 1))), catalog, rules)
        ...
        assertFailsWith<IllegalArgumentException> { WarbandCore.validate(state, catalog, rules) }
    }
    ...
    @Test fun `extraction values marginal progress toward missing armament materials`() {
        ...
        assertEquals("needed", WarbandCore.chooseMaterial(state, warband, logisticalCatalog, rules)?.id)
    }
    ...
    @Test fun `environment changes formulaic part material preference without material archetypes`() {
        ...
        assertEquals("durable", WarbandCore.choosePartMaterial(state, warband, materials, materials.mapTo(linkedSetOf()) { it.id }, available, 1.0, 0, rules)?.id)
        ...
        assertEquals("ranged", WarbandCore.choosePartMaterial(state, warband, materials, materials.mapTo(linkedSetOf()) { it.id }, available, 1.0, 0, rules)?.id)
    }

    @Test fun `catalog and frames fail closed`() {
        assertFailsWith<IllegalArgumentException> { WarbandCore.transition(state(), CoreFrame(-1L), catalog, rules) }
        assertFailsWith<IllegalArgumentException> { WarbandCore.transition(state(), CoreFrame(0L), catalog.copy(revision = ""), rules) }
        assertFailsWith<IllegalArgumentException> {
            WarbandCore.transition(state(), CoreFrame(0L), catalog.copy(resources = listOf(ResourceDefinition("bad", ResourceVector(), environmentalAvailability = -1.0))), rules)
        }
        assertNotNull(WarbandCore.chooseRecruit(state(), state().warbands.getValue("warband"), null, catalog, 6.0))
    }
    ...
    @Test fun `selection history changes formulaic recruit choice without fixed rosters`() {
        ...
        assertEquals("second", WarbandCore.chooseRecruit(state, warband, null, equalCatalog, 5.0, rules = rules)?.id)
    }
    ...
    @Test fun `equipment assignment maximizes member utility instead of armory order`() {
        ...
        val plan = WarbandCore.planSquad(state, warband, null, meleeOnly, 5.0, rules)
    ...
    @Test fun `defensive and utility equipment remain functional without hard coded recruit kits`() {
        ...
        val plan = WarbandCore.planSquad(state, warband, null, catalog.copy(recruits = listOf(catalog.recruits.last())), 12.0, rules)
    ...
    @Test fun `partial materialization restores each failed manifest exactly once`() {
        ...
        failed.cargo["ration"] = 3
        failed.equipment = EquipmentManifest("partial-tool", "blade", emptyList(), emptyMap(), CapabilityVector(damage = 1.0))
        val beforePool = state.warbands.getValue("warband").raidPool
        val effect = CoreEffect(
            EffectKind.MATERIALIZE, "warband", campaign.id, "player", campaign.position,
            campaign.members.map { it.id }, effectId = "effect:partial",
        )
        state.pendingEffects[effect.effectId] = effect
        val result = MaterializationResult(campaign.id, true, setOf(successful.id), effect.effectId)

        WarbandCore.transition(state, CoreFrame(0L, materializations = listOf(result)), catalog, rules)
        val afterFirst = Json.encodeToString(state)
        assertEquals(listOf(successful.id), campaign.members.map { it.id })
        assertEquals(setOf(successful.id), campaign.physicalMemberIds)
        assertEquals(beforePool + failed.threat, state.warbands.getValue("warband").raidPool)
        assertEquals(3, state.warbands.getValue("warband").stockpile["ration"])
        assertTrue(state.warbands.getValue("warband").armory.any { it.id == "partial-tool" })
        assertTrue(effect.effectId in state.acknowledgedEffectIds)

        WarbandCore.transition(state, CoreFrame(0L, materializations = listOf(result)), catalog, rules)
        assertEquals(afterFirst, Json.encodeToString(state))
    }
    ...
    @Test fun `discovery territory and garrison reservations are core owned`() {
        ...
        WarbandCore.transition(
            state,
            CoreFrame(0L, garrisonResults = listOf(GarrisonResult(garrison.id, false, effectId = effect.effectId))),
            catalog,
            rules,
        )
    ...
    @Test fun `tactical intent and successor selection are deterministic core effects`() {
        ...
        val campaign = state.campaigns.values.single().also {
            ...
            it.physical = true
            it.physicalMemberIds += it.members.map(MemberManifest::id)
        }
        val exposed = TacticalPosition("exposed", ChunkPosition("overworld", 2, 0), 1.0, 5.0)
        val covered = TacticalPosition("covered", ChunkPosition("overworld", 1, 1), 1.0, 5.0, cover = 2.0)
        val result = WarbandCore.transition(
        ...
        assertEquals(2, state.officers.getValue("veteran").rank)
        WarbandCore.transition(
            state,
            CoreFrame(0L, commands = listOf(CoreCommand.CollapseWarband("warband", "test"))),
            catalog,
            rules,
        )
        assertTrue(state.warbands.getValue("warband").defeated)
        assertEquals(CampaignPhase.RESOLVED, campaign.phase)
        assertTrue(state.pendingEffects.values.none { it.warbandId == "warband" })
    }

    @Test fun `garrison snapshots conserve surviving threat and exact equipment`() {
        val state = state(reserve = 12.0, pool = 0.0)
        val warband = state.warbands.getValue("warband")
        val reserved = WarbandCore.transition(
            state,
        ...
        val materialize = reserved.effects.single { it.kind == EffectKind.MATERIALIZE_GARRISON }
        WarbandCore.transition(
            state,
            CoreFrame(0L, garrisonResults = listOf(GarrisonResult(garrison.id, true, garrison.members.mapTo(linkedSetOf()) { it.id }, materialize.effectId))),
            catalog,
            rules,
        )
        val survivor = garrison.members.first()
        val casualtyThreat = garrison.members.drop(1).sumOf(MemberManifest::threat)
        val recoveredTool = EquipmentManifest("recovered", "blade", emptyList(), emptyMap(), CapabilityVector(damage = 1.0), durabilityFraction = 0.4)
        WarbandCore.transition(
            state,
            CoreFrame(0L, garrisonSnapshots = listOf(GarrisonSnapshotResult(
                garrison.id,
                listOf(MemberSnapshot(survivor.id, 0.5, 2.0, recoveredTool, mapOf("ration" to 1))),
            ))),
            catalog,
            rules,
        )
        assertEquals(casualtyThreat, 12.0 - warband.reserveThreat - warband.garrisonThreat)
        ...
        assertTrue(warband.armory.any { it.id == recoveredTool.id && it.durabilityFraction == 0.4 })
        assertEquals(1, warband.stockpile["ration"])
    }

    @Test fun `rewards and faction collapse are formulaic idempotent Core decisions`() {
        val state = state(pool = 12.0)
        val rewardCatalog = catalog.copy(rewards = listOf(
        ...
        val first = WarbandCore.transition(state, CoreFrame(0L, defeats = listOf(observation)), rewardCatalog, rules)
        val reward = first.effects.single { it.kind == EffectKind.REWARD_PLAYER }
        assertEquals("coin:gold", reward.itemId)
        assertEquals(kotlin.math.ceil(member.threat * 2.0 / 5.0).toInt(), reward.count)
        WarbandCore.transition(
            state,
            CoreFrame(0L, acknowledgements = listOf(EffectAcknowledgement(reward.effectId)), defeats = listOf(observation)),
            rewardCatalog,
            rules,
        )
        assertTrue(state.pendingEffects.values.none { it.kind == EffectKind.REWARD_PLAYER })

        WarbandCore.transition(
            state,
            CoreFrame(0L, commands = listOf(CoreCommand.CollapseFaction("faction", "warlord_defeated"))),
            rewardCatalog,
            rules,
        )
        assertTrue(state.warbands.values.all(WarbandState::defeated))
        assertTrue(state.officers.values.all { it.availableAtTick == Long.MAX_VALUE })
    }

    @Test fun `failed effect attempts retry and successful acknowledgement is exactly once`() {
        val state = state()
        val effect = CoreEffect(EffectKind.WARN_PLAYER, "warband", playerId = "player", effectId = "effect:retry")
        state.pendingEffects[effect.effectId] = effect

        val failed = WarbandCore.transition(
            state,
            CoreFrame(0L, acknowledgements = listOf(EffectAcknowledgement(effect.effectId, false, "chunk unavailable"))),
            catalog,
            rules,
        )
        assertEquals(effect, state.pendingEffects[effect.effectId])
        assertTrue(failed.events.any { it.type == "effect_attempt_failed" })

        WarbandCore.transition(
            state,
            CoreFrame(0L, acknowledgements = listOf(EffectAcknowledgement(effect.effectId))),
            catalog,
            rules,
        )
        val afterSuccess = Json.encodeToString(state)
        WarbandCore.transition(
            state,
            CoreFrame(0L, acknowledgements = listOf(EffectAcknowledgement(effect.effectId))),
            catalog,
        ...
        assertEquals(afterSuccess, Json.encodeToString(state))
        assertTrue(effect.effectId in state.acknowledgedEffectIds)
    }
    ...
    @Test fun `all attempted materialization failures restore exact manifests and resolve`() {
        ...
        val beforePool = state.warbands.getValue("warband").raidPool
        campaign.members.first().cargo["ration"] = 2
        val effect = CoreEffect(
            EffectKind.MATERIALIZE, "warband", campaign.id, "player", campaign.position,
            campaign.members.map(MemberManifest::id), effectId = "effect:all-failed",
        )
        state.pendingEffects[effect.effectId] = effect
        val attempted = campaign.members.mapTo(linkedSetOf(), MemberManifest::id)

        WarbandCore.transition(
            state,
            CoreFrame(0L, materializations = listOf(MaterializationResult(campaign.id, false, effectId = effect.effectId, attemptedMemberIds = attempted))),
            catalog,
            rules,
        )
        assertEquals(CampaignPhase.RESOLVED, campaign.phase)
        assertTrue(campaign.members.isEmpty())
        assertEquals(beforePool + committed, state.warbands.getValue("warband").raidPool)
        assertEquals(2, state.warbands.getValue("warband").stockpile["ration"])
        assertTrue(effect.effectId in state.acknowledgedEffectIds)
    }

    @Test fun `player scheduling commands and reset are Core owned`() {
        val state = state()
        WarbandCore.transition(
            state,
        ...
        assertTrue("player" in state.initializedPlayerIds)
        assertEquals(40L, state.protectedPlayersUntilTick["player"])
        assertEquals(7L, state.lastDiscoveryTick)
        assertEquals(9L, state.lastCampaignTick)

        WarbandCore.transition(state, CoreFrame(41L), catalog, rules)
        assertTrue("player" !in state.protectedPlayersUntilTick)
        WarbandCore.transition(state, CoreFrame(0L, commands = listOf(CoreCommand.ResetWorld)), catalog, rules)
        assertTrue(state.warbands.isEmpty())
        assertTrue(state.initializedPlayerIds.isEmpty())
