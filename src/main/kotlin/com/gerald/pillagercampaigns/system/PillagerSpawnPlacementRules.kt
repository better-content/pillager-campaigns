...
object PillagerAttemptRules {
    ...
    fun commandFeedback(spawnedGroups: Int, status: String): String =
        "Pillager Campaigns forced campaign attempt spawned_groups=$spawnedGroups status=$status"
}
...
object PillagerObjectiveRules {
    ...
    fun objectiveFor(campaign: com.gerald.pillagercampaigns.data.PillagerCampaign?, target: net.minecraft.server.level.ServerPlayer?, fallback: BlockPos): Objective {
        if (target != null) return Objective("player", target.blockPosition())
        if (campaign != null) {
            val kind = when (campaign.state) {
                com.gerald.pillagercampaigns.data.CampaignState.SCOUTING -> "scout"
                com.gerald.pillagercampaigns.data.CampaignState.APPROACHING_INTEL -> "hunt_intel"
                com.gerald.pillagercampaigns.data.CampaignState.SEARCHING -> "search"
                com.gerald.pillagercampaigns.data.CampaignState.ENGAGING -> "engage"
                com.gerald.pillagercampaigns.data.CampaignState.RETREATING_WITH_INTEL -> "retreat"
                com.gerald.pillagercampaigns.data.CampaignState.RETURNING_TO_BASE -> "return"
                com.gerald.pillagercampaigns.data.CampaignState.EXPANDING -> "expand"
                com.gerald.pillagercampaigns.data.CampaignState.SUPPLYING -> "supply"
                com.gerald.pillagercampaigns.data.CampaignState.DISBANDED -> "disbanded"
            }
